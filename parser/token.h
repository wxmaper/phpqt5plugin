...
public:
    static Token getIdentifierToken(const SourceCodeStream &stream);
    static Token getOperatorToken(const SourceCodeStream &stream);
    ...
    Token(int b, int l, Token::T t, const QString &value = QStringLiteral(""))
        : m_begin(b), m_length(l), m_type(t), m_value(value)
    {}
    ...
    bool is(T type) const { return type == m_type; }

    void clear() {
        m_begin = 0; m_length = 0;
        m_type = T_NONE;
        m_value = QStringLiteral("");
    }
...
#include <QDebug>
#include <QMetaEnum>
QDebug inline operator<< (QDebug d, const PHPQt5::Internal::Token &t)
{
    QMetaEnum metaEnum = QMetaEnum::fromType<PHPQt5::Internal::Token::T>();
    QString dstr = QString("Token::%1(`%2`, begin:%3, len:%4)")
            .arg(QLatin1String(metaEnum.valueToKey(t.type())))
            .arg(t.value().replace("%","---!PERCENT!---"))
            .arg(t.begin())
            .arg(t.length());

    d.maybeSpace() << dstr.replace("---!PERCENT!---", "%");
